...
export default async function permisoPost(req: NextApiRequest, res: NextApiResponse) {
    if (req.method !== 'POST') {
        ...
        try {
            ...
            const [rows] = await conn.query('CALL crear_constancia(?)', [numero_de_control]);
            const resultado = rows[0]; 
            if (resultado.length > 0) {
                const { folio_constancia_creado } = resultado[0];
                return res.status(200).json({ folio: folio_constancia_creado });
            } else {
                return res.status(400).json({ error: "No se pudo obtener el folio de la constancia" });
            }
        } catch (error) {
